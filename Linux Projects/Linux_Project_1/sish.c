...
void historyOffset(long offset) {
    //printf("This is the offset: %ld\n", offset);

    if (offset >= 0 && offset < HISTORY_SIZE) { // Ensure offset is within valid range
        if (historyCount < HISTORY_SIZE) {
            char* commandCopy = strdup(commandHistory[offset]);
            tokenizer(commandCopy);
            free(commandCopy);
        } else {
            int adjustedOffset = (commandHistoryIndex + offset - 1) % HISTORY_SIZE; // ajusts the proper offset for the circular array
            char* commandCopy = strdup(commandHistory[adjustedOffset]); //makes sure that commandHistory doesn't get messed up when accessing the index
            tokenizer(commandCopy); //runs the command
            free(commandCopy);
        }
    } else {
